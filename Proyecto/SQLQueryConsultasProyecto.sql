...
#ClienteFrecuente.Direccion, Cine.Nombre
...
CREATE TABLE
#TotalDeProducto
(Producto nvarchar(50) , NumeroCompras int ,PrecioProducto numeric(8,2), Descuento numeric(8,2), NuevoPrecio numeric(8,2))
DECLARE @Producto nvarchar(50)
DECLARE @NumeroCompras int
DECLARE @PrecioProducto numeric(8,2)
DECLARE @Descuento numeric(8,2)
DECLARE @NuevoPrecio numeric(8,2)

DECLARE CursorProducto CURSOR FOR
SELECT ProductoDulceria.Producto as Producto, count(ProductoDulceria.IDCompra),sum(ProductoDulceria.Precio) as TotalPrecioProducto,
CASE
    WHEN count(ProductoDulceria.IDCompra) between 0 and 5 then 0.00
    WHEN count(ProductoDulceria.IDCompra) between 6 and 10 then 0.05
    WHEN count(ProductoDulceria.IDCompra) between 11 and 15 then 0.10
    WHEN count(ProductoDulceria.IDCompra) > 15 then 0.15
END as Descuento
,
CASE
    WHEN count(ProductoDulceria.IDCompra) between 0 and 5 then ProductoDulceria.Precio
    WHEN count(ProductoDulceria.IDCompra) between 6 and 10 then ProductoDulceria.Precio - ProductoDulceria.Precio*0.05
    WHEN count(ProductoDulceria.IDCompra) between 11 and 15 then ProductoDulceria.Precio - ProductoDulceria.Precio*0.10
    WHEN count(ProductoDulceria.IDCompra) > 15 then ProductoDulceria.Precio  - ProductoDulceria.Precio*0.15
END as NuevoPrecio
FROM ProductoDulceria inner join Compra on ProductoDulceria.IDCompra = Compra.IDCompra
Group By ProductoDulceria.Producto, ProductoDulceria.Precio
OPEN CursorProducto
    FETCH NEXT FROM CursorProducto into @Producto,@NumeroCompras,@PrecioProducto,@Descuento,@NuevoPrecio
WHILE @@FETCH_STATUS = 0
BEGIN
    INSERT INTO #TotalDeProducto
    (Producto,NumeroCompras,PrecioProducto,Descuento,NuevoPrecio)
    VALUES (@Producto,@NumeroCompras,@PrecioProducto,@Descuento,@NuevoPrecio)
    FETCH NEXT FROM CursorProducto INTO @Producto, @NumeroCompras, @PrecioProducto,@Descuento,@NuevoPrecio
END
CLOSE CursorProducto
DEALLOCATE CursorProducto
SELECT * FROM #TotalDeProducto
DROP TABLE #TotalDeProducto
